...
Make it authentic to your personality. Be creative and expressive!`;
  ...
  // Generate profile picture if doesn't exist (non-fatal — profile still works without it)
  if (!npc.profile_image_url) {
    try {
      ...
      console.warn(`[Profile Populator] Image generation failed for ${npc.display_name}: ${error.message}`);
      console.warn('[Profile Populator] Continuing without profile picture (no image gen provider configured?)');
    }
  }
