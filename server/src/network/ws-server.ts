...
async function routeMessage(
  ...
  switch (message.type) {
    ...
    // Media routes
    case 'media:get':
      await handleMediaGet(ws, message);
      break;

    case 'media:getAll':
      await handleMediaGetAll(ws, message);
      break;

    // Studio routes
    case 'studio:generateImage':
      await handleStudioGenerateImage(ws, message);
      break;

    case 'studio:getBudget':
      await handleStudioGetBudget(ws, message);
      break;

    case 'studio:saveVideoConfig':
...
async function handleMediaGet(ws: ServerWebSocket<ClientSession>, message: WSMessage): Promise<void> {
  if (!message.payload || !(message.payload as any).id) {
    send(ws, createResponse(message.id, false, null, 'Missing media id'));
    return;
  }
  ...
  if (!file) {
    send(ws, createResponse(message.id, false, null, 'Media file not found'));
    return;
  }

  send(ws, createResponse(message.id, true, file));
}

async function handleMediaGetAll(ws: ServerWebSocket<ClientSession>, message: WSMessage): Promise<void> {
...
async function handleStudioGenerateImage(ws: ServerWebSocket<ClientSession>, message: WSMessage): Promise<void> {
  const payload = message.payload as {
    prompt: string;
    style?: string;
    mood?: string;
  ...
  if (!payload?.prompt) {
    send(ws, createResponse(message.id, false, null, 'Missing prompt'));
    return;
  }
  ...
  try {
    ...
    send(ws, createResponse(message.id, false, null, err.message || 'Image generation failed'));
  }
}

async function handleStudioGetBudget(ws: ServerWebSocket<ClientSession>, message: WSMessage): Promise<void> {
  ...
  try {
    ...
    send(ws, createResponse(message.id, false, null, err.message || 'Failed to get budget'));
  }
}

async function handleStudioSaveVideoConfig(ws: ServerWebSocket<ClientSession>, message: WSMessage): Promise<void> {
  const payload = message.payload as {
  ...
  if (!payload?.config) {
    send(ws, createResponse(message.id, false, null, 'Missing video config'));
    return;
  }
