...
export function LinkedCornSite({ siteId, path, onNavigate, onPathChange }: SiteProps) {
  ...
  const profiles = useMemo(() => dbProfiles.map(dbToProfile), [dbProfiles])

  const posts = useMemo(() => dbPosts.map(item => dbToPost(item, profiles)), [dbPosts, profiles])

  const jobs = useMemo(() => dbJobs.map(dbToJob), [dbJobs])
