...
/**
 * PinballWindow Component
 *
 * Main pinball interface with three tabs:
 * - Game: Play Space Cadet Pinball
 * - Leaderboard: ELO rankings
 * - History: Game history and stats
 */

type Tab = 'game' | 'leaderboard' | 'history';
...
export function PinballWindow() {
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--color-bg)]">
      ...
      >
        <h1 className="text-2xl font-bold text-[var(--color-text)] mb-1">Space Cadet Pinball</h1>
        <p className="text-sm text-[var(--color-textMuted)]">
          Beat the benchmark to climb the ELO leaderboard
        </p>
      </div>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {activeTab === 'game' && <PinballGame />}
        {activeTab === 'leaderboard' && <PinballLeaderboard />}
        {activeTab === 'history' && <PinballHistory />}
      </div>
    </div>
  );
}
